...
# FFFFF|FFFFG
### init
## 100 * 100 , one-hot encoding = 10000
## 100+100 =200
## 400 * 400 -> 800
...
for(i in 1:30000){
  ...
  while(episode_done==0){
    ...
    real_current_state<-convert_coord(st)
    ...
    memory[[epoch_index]]<-list(input=(real_current_state),next_state=cov_next_state,action=next_action,done=re_ep[2],reward=re_ep[1])
    ...
    if(episode_done | step %% epoch == 0){
      ...
      if(episode_done){
        ...
        vs<-  nn.ff2(vg,t(real_current_state))$post[[length(size)]]
      }
      ...
      yy2<-yy
      for(p in 1:nrow(yy2)){
        yy[p,action %in%action_stack[p]] <- log(yy[p,action %in%action_stack[p]]+0.0001) *td_error[p]
        yy2[p,action %in%action_stack[p]]<-      yy2[p,action %in%action_stack[p]]*  log(yy2[p,action %in%action_stack[p]])
      }
      
      exp_v<- -yy 
      entropy<- -yy2 
      qn1$e<- (0.01 * entropy + exp_v)
      qn1<-nn.bp(qn1)
      memory<-list()
      epoch_index<-1
    }
